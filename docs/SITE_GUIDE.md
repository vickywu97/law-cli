...
│   └── 06_执行检查表.md             ← 待办闭环跟踪
└── reports/                         ← 工程与溯源报告（已在 /docs 内，纳入 Pages 站点）
...
根级文件（仓库根，不在 `/docs` 站点树内，走 GitHub blob）：`README.md` / `README_EN.md`（英文）/ `LICENSE` / `CONTRIBUTING.md` / `SECURITY.md` / `pyproject.toml` / `RELEASE_NOTES.md`。
